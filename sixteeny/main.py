...
if __name__ == '__main__':
    ...
    MFCController(
        ...
        for i in range(len(rig_config['mfc_device_ids'])):
            if average_flow_rates[i] < rig_config['mfc_flow_rate']*(1-error_margin) or average_flow_rates[i] > rig_config['mfc_flow_rate']*(1+error_margin):
                print('Flow rate for MFC ' + str(i) + ' is out of range. Expected ' + str(rig_config['mfc_flow_rate']) + ' mL/min, got ' + str(average_flow_rates[i]) + ' mL/min.')
                sys.exit(1)
        
        print('MFC values are within {}% error margin based on {} observations.'.format(error_margin*100, n_observations))
        ...
        for i in experiment_files:
            ...
            if experiment_file['fly_experiment'].endswith('.csv'):
                experimenters[i] = CSVExperimenter(project_directory + experiment_name + '/experiments/' + experiment_file['fly_experiment'])
            else:
        ...
        while experiment_ongoing:
            ...
            for i in range(n_objects):
                ...
                area = regions[i].area
                in_reward_region = reward_mask[:,position_y, position_x].any and np.argmax(reward_values) == np.argmax(arm_values)
            ...
            for i in fly_arenas:
                ...
                position_x, position_y = object_summary[object_summary[:,2] == i,0][largest_object], object_summary[object_summary[:,2] == i,1][largest_object]
