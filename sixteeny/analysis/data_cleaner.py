...
def process_file(file_path, output_path, estimated_transforms, origin):
    ...
    instantaneous_upwind_speed = np.diff(instantaneous_radial_distance, axis=0) / np.diff(data["frame_times"], axis=0)
    ...
    current_odor = np.array(
        [current_odor_vectors[i][np.int32(np.array(data["current_arms"])[i])] for i in range(len(current_odor_vectors))]
    )
    ...
    trial_oriented_position = np.array(
        [realign_lr(current_start_arms[i], reference_fly_positions[i], index) for i in range(len(data["frame_times"]))]
    ).squeeze()
