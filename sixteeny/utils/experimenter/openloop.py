...
class CSVExperimenter(Experimenter):
    ...
    def get_next_trial(self, history):
        ...
        next_trial = {}
        next_trial['reward_probability'] = list(self.experiment_config.iloc[self.trial_number][['P(R|Air)','P(R|O1)','P(R|O2)']])
        next_trial['relative_odor_vector'] = list(self.experiment_config.iloc[self.trial_number][['Odor(Start)','Odor(Left)','Odor(Right)']])
        next_trial['time_needed_in_reward_zone'] = list(self.experiment_config.iloc[self.trial_number][['StayTime(Air)','StayTime(O1)','StayTime(O2)']])
        next_trial['reward_stimulus'] = list(self.experiment_config.iloc[self.trial_number][['CStim(Air)','CStim(O1)','CStim(O2)']])
