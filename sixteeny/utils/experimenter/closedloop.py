...
class DeterministicMultilevelExperimenter(Experimenter):
    """
    A class that uses a Markov chain on State-Action space to design experiments with multiple levels of reward
    """
    def __init__(self, experiment_config_file):
        """
        A method to initialize a DeterministicMultilevelExperimenter object
        """
        super().__init__(experiment_config_file)
        # confirm that the config file is a json object
        assert self.experiment_config_file.endswith(".ymle")
        ...
        self.state_labels = np.array(self.experiment_config["state_labels"]) # 0 is unrewarded, 1 is rewarded (low), 2 is rewarded (mid), 3 is rewarded (high)
        self.transition_matrix = np.array(
            self.experiment_config["state_transitions"]
        )  # shape (n_states, 2)
        self.rewarded_low_stimulus = self.experiment_config["rewarded_low_stimulus"]
        self.rewarded_mid_stimulus = self.experiment_config["rewarded_mid_stimulus"]
        self.rewarded_high_stimulus = self.experiment_config["rewarded_high_stimulus"]
        ...
        # initialize the current state (starting state is always 0)
    ...
    def __reward_alloc(self, reward):
        """
        A method to allocate reward stimuli
        """
        if reward == 0:
            return self.unrewarded_stimulus
        elif reward == 1:
            return self.rewarded_low_stimulus
        elif reward == 2:
            return self.rewarded_mid_stimulus
        elif reward == 3:
            return self.rewarded_high_stimulus
...
class ProbabilisticFiniteStateExperimenter(Experimenter):
    ...
    def __init__(self, experiment_config_file):
        ...
        super().__init__(experiment_config_file)
        # confirm that the config file is a json object
        assert self.experiment_config_file.endswith(".ypse")
        # read the config file
        with open(self.experiment_config_file) as f:
            self.experiment_config = json.load(f)
        # get experiment folder as the full path to the folder containing the experiment config file
