...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def get_experiments_from_directory(self, directory):
        ...
        if not os.path.exists(directory+'/experiment_zoo'):
            ...
            experiments = [experiment for experiment in experiments if experiment.endswith('.csv') or experiment.endswith('.py')]
    ...
    def verify_start_experiment(self):
        ...
        while not config_present:
            ...
            if 'config.yarena' not in files:
                ...
                self.config_file = os.path.join(self.project_directory.text(), experiment_name, [file for file in files if file.endswith('.yarena')][0])
        ...
        for experiment in used_experiments:
            ...
            if os.path.isfile(os.path.join(self.project_directory.text(), 'experiment_zoo', experiment.split('.')[0]+'.meta')):
                ...
                if 'used_stimuli' not in metadata:
                    ...
                    QtWidgets.QMessageBox.warning(self, 'Invalid Metadata File', 'The metadata file for the experiment: "'+experiment+'" is invalid.')
                    return False
                else:
