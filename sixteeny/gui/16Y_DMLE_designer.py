...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, start_folder=None, load_file=None, *args, **kwargs):
        super().__init__()
        self.setWindowTitle("Deterministic Multilevel Experiment Designer")
        ...
        default_database = "C:\\16YArena\\TurnerLab_Opto2AFC_16Y\\sixteeny\\utils\\experimenter\\DMLEtasks.pkl"
        try:
        ...
        # create a text label, text box, and button to load stimulus for reward stimulus (low)
        self.rewarded_low_label = QtWidgets.QLabel("Rewarded Stimulus (Low)")
        self.main_layout.addWidget(self.rewarded_low_label, 5, 0, 1, 1)
        self.rewarded_low_textbox = QtWidgets.QLineEdit("empty.stim")
        self.rewarded_low_button = QtWidgets.QPushButton("Load")
        self.rewarded_low_button.clicked.connect(self.load_rewarded_low)
        self.rewarded_low_textbox.setReadOnly(True)
        self.main_layout.addWidget(self.rewarded_low_textbox, 5, 1, 1, 2)
        self.main_layout.addWidget(self.rewarded_low_button, 5, 3, 1, 1)

        # create a text label, text box, and button to load stimulus for reward stimulus (mid)
        self.rewarded_mid_label = QtWidgets.QLabel("Rewarded Stimulus (Mid)")
        self.main_layout.addWidget(self.rewarded_mid_label, 6, 0, 1, 1)
        self.rewarded_mid_textbox = QtWidgets.QLineEdit("empty.stim")
        self.rewarded_mid_button = QtWidgets.QPushButton("Load")
        self.rewarded_mid_button.clicked.connect(self.load_rewarded_mid)
        self.rewarded_mid_textbox.setReadOnly(True)
        self.main_layout.addWidget(self.rewarded_mid_textbox, 6, 1, 1, 2)
        self.main_layout.addWidget(self.rewarded_mid_button, 6, 3, 1, 1)

        # create a text label, text box, and button to load stimulus for reward stimulus (high)
        self.rewarded_high_label = QtWidgets.QLabel("Rewarded Stimulus (High)")
        self.main_layout.addWidget(self.rewarded_high_label, 7, 0, 1, 1)
        self.rewarded_high_textbox = QtWidgets.QLineEdit("empty.stim")
        self.rewarded_high_button = QtWidgets.QPushButton("Load")
        self.rewarded_high_button.clicked.connect(self.load_rewarded_high)
        self.rewarded_high_textbox.setReadOnly(True)
        self.main_layout.addWidget(self.rewarded_high_textbox, 7, 1, 1, 2)
        self.main_layout.addWidget(self.rewarded_high_button, 7, 3, 1, 1)
        ...
        self.main_layout.addWidget(self.unrewarded_label, 8, 0, 1, 1)
        ...
        self.main_layout.addWidget(self.unrewarded_textbox, 8, 1, 1, 2)
        self.main_layout.addWidget(self.unrewarded_button, 8, 3, 1, 1)
        ...
        self.main_layout.addWidget(self.naive_trials_label, 9, 0, 1, 1)
        self.main_layout.addWidget(self.naive_trials_textbox, 9, 1, 1, 1)
        ...
        self.num_trials_textbox.setText("1500")
        self.num_trials_textbox.setValidator(QtGui.QIntValidator())
        self.main_layout.addWidget(self.num_trials_label, 9, 2, 1, 1)
        self.main_layout.addWidget(self.num_trials_textbox, 9, 3, 1, 1)

        # create a button to save the experiment
        self.save_button = QtWidgets.QPushButton("Save")
        self.save_button.clicked.connect(self.save)
        self.main_layout.addWidget(self.save_button, 10, 0, 1, 2)

        # create a button to load the experiment
        self.load_button = QtWidgets.QPushButton("Load")
        self.load_button.clicked.connect(self.load)
        self.main_layout.addWidget(self.load_button, 10, 2, 1, 2)
    ...
    def load_rewarded_low(self):
        ...
        if filename:
            ...
            self.rewarded_low_textbox.setText(filename.split("/")[-1])
    
    def load_rewarded_mid(self):
        """
        Load the stimulus for odor 1.
        """
        folder = self.start_folder
        ...
        if filename:
            # set the textbox to the file name (without the path)
            self.rewarded_mid_textbox.setText(filename.split("/")[-1])
    
    def load_rewarded_high(self):
        """
        Load the stimulus for odor 1.
        """
        folder = self.start_folder
        ...
        if filename:
            # set the textbox to the file name (without the path)
            self.rewarded_high_textbox.setText(filename.split("/")[-1])
    ...
    def load_task(self):
        ...
        if self.task_id in self.tasks.index:
            # get the task
            self.task = self.tasks.loc[self.task_id]['task']
            # set the text boxes
            self.num_states = len(self.task[0])
            self.num_states_textbox.setText(str(self.num_states))
            self.state_labels = np.array(self.task[0], dtype=np.int32)
    ...
    def save(self):
        ...
        if self.load_file is not None:
            ...
            filename, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save Experiment", default_folder, "*.ymle")
        ...
        if filename:
            ...
            experiment = {
                ...
                "state_labels": self.state_labels.tolist(),
                "state_transitions": self.state_transitions.tolist(),
                "rewarded_low_stimulus": self.rewarded_low_textbox.text(),
                "rewarded_mid_stimulus": self.rewarded_mid_textbox.text(),
                "rewarded_high_stimulus": self.rewarded_high_textbox.text(),
            ...
            if self.reciprocal_task.isChecked():
                ...
                with open(filename[:-5]+"_reciprocal.ymle", "w") as f:
                    json.dump(experiment, f)
        ...
        stimuli = {"used_stimuli": [self.unrewarded_textbox.text(), self.rewarded_low_textbox.text(), self.rewarded_mid_textbox.text(), self.rewarded_high_textbox.text()]}
        ...
        reply = QtWidgets.QMessageBox.question(
            self,
            "Close",
            "Do you want to close the Deterministic Multilevel Experiment Generator?",
    ...
    def load(self, load_file=None):
        """
        Load an experiment from a ymle file.
        """
        ...
        # otherwise, open a file dialog
            ...
            filename, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Load Experiment", default_folder, "*.ymle")
        ...
        if filename:
            ...
            state_transitions_str = state_transitions_str[:-1]+"]"
            self.state_transitions_textbox.setText(state_transitions_str)
            self.rewarded_low_textbox.setText(experiment["rewarded_low_stimulus"])
            self.rewarded_mid_textbox.setText(experiment["rewarded_mid_stimulus"])
            self.rewarded_high_textbox.setText(experiment["rewarded_high_stimulus"])
